...
# use NODE_ENV=development PORT=8082 to avoid conflicting with anything else
NODE_ENV=development PORT=8082 node server.js &
SERVER_PID=$!
...
COURSE_ID=$(echo "$COURSE_RESP" | grep -oP '"_id":"\K([^"]+)')
